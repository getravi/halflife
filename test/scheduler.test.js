...
// Thin shims so the original assertions read exactly as they did under
// node:test. The point of these tests is the reasoning in their names, not
// the assertion dialect.
const expect_eq = (a, b) => expect(a).toBe(b);
const expect_deep = (a, b) => expect(a).toEqual(b);
const expect_ok = (a, msg) => expect(a, msg).toBeTruthy();
const expect_throws = (fn, re) => expect(fn).toThrow(re);
...
test('a brand-new card is due immediately, because unreviewed is not the same as known', () => {
  const c = make();
  expect_eq(S.isDue(c, T0), true);
  expect_eq(S.retrievability(c, T0), 0);
});

test('grading good the first time schedules four days out', () => {
  const c = S.review(make(), 'good', T0);
  expect_eq(c.stability, 4);
  expect_eq(c.dueAt, T0 + 4 * DAY);
  expect_eq(c.reps, 1);
  expect_eq(c.lapses, 0);
});

test('retrievability is exactly 0.9 at the due date, because that is what due means', () => {
  const c = S.review(make(), 'good', T0);
  expect_ok(Math.abs(S.retrievability(c, c.dueAt) - 0.9) < 1e-9);
  expect_eq(S.isDue(c, c.dueAt - 1), false);
  expect_eq(S.isDue(c, c.dueAt), true);
});
...
test('retrievability decays continuously between reviews, so a stale card reads as stale', () => {
  ...
  expect_ok(r1 > r2, 'later means lower');
  expect_ok(r1 < 1, 'a day after review you no longer hold it perfectly');
});
...
test('stability never falls below one day, so a card cannot become due in the past', () => {
  ...
  expect_ok(c.stability >= 1);
  expect_ok(c.dueAt > T0 + 5 * DAY);
});
